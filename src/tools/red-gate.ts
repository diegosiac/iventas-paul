...
export function registerRedGateTool(server: McpServer, client: PaulClient): void {
  server.registerTool(
    "paul_resolve_red_gate",
    {
      title: "Resolve a PAUL red gate (team-visible time flag)",
      description:
        "Resolve a red gate — the team-visible time-hygiene flag PAUL raises " +
        ...
        return textResult(
          {
            error: true,
            message:
              `Plan too short (${plan.trim().length} chars): PAUL requires a plan of at ` +
