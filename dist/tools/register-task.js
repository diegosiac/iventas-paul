 ...
 */
const URGENCY_WORD_RE = /\b(alta|urgente|urge|prioritario|media|normal|moderad\w*|regular|baja|sin prisa|cuando puedas|no urge|low)\b/i;
/**
...
function titlesMatch(requested, stored) {
 ...
 */
export async function registerTask(client, title, urgency) {
    ...
    if (urgencyWord !== undefined) {
        return {
            ok: false,
            titleRejected: true,
            error: `The title contains an urgency-trigger word ("${urgencyWord}") that could make ` +
    ...
    try {
        ...
        if (typeof first.undo_id === "number")
            undoId = first.undo_id;
        if (first.assigned !== true && asksForUrgency(first.reply)) {
            ...
            if (typeof second.undo_id === "number")
                undoId = second.undo_id;
        }
    }
    catch (err) {
...
export function registerRegisterTaskTool(server, client) {
    server.registerTool("paul_register_task", {
        ...
        description: "Register a NEW task for yourself in PAUL via its coach chat dialogue. " +
